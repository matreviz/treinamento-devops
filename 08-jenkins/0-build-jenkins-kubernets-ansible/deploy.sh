#!/bin/bash
cd ~/treinamento-devops/08-jenkins/0-build-jenkins-kubernets-ansible/
cd 0-terraform
terraform init
terraform fmt
...
echo "$(cd  ~/treinamento-devops/08-jenkins/0-build-jenkins-kubernets-ansible/0-terraform & terraform output | grep public_dns | awk '{print $2;exit}')" | sed -e "s/\",//g" >> ../1-ansible/hosts # captura output faz split de espaco e replace de ",
